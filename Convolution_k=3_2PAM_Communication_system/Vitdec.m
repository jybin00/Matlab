...
%----------------------------------------------------------------------------------------
outputs = trel.outputs;
output_zero = int2bit(outputs(:,1)', n_inputSymbols)';
...
addRequired(p, 'trel', @istrellis);
addRequired(p, 'num_message_bit', @isnumeric);
...
%% hard decoding function
    function decoded_output = hard_vitdec(demodulated_output)
        ...
        %---------------------------------------------------------------------------------
        % input을 2개씩 끊어서 received_vector 구성 (R = 1/2 이므로)
        received_vector = reshape(demodulated_output, [n_inputSymbols, num_message_bit + tail_bit]);
        ...
        for t = 1 : num_message_bit + tail_bit + 1
            ...
            elseif t > 1 && t <= n_mem
                for j = 1 : 2^(t-1)
                    ...
                    if Current_state < n_states / n_inputSymbols
                        BM_zero = nnz(output_zero(Prev_state_zero, :) - received_vector(t-1,:));   % 끝자리가 0인 코드에 0이 들어왔을 때 outputd으로 BM 계산
                        BM_one  = nnz(output_zero(Prev_state_one,  :) - received_vector(t-1, :));  % 끝자리가 1인 코드에 0이 들어왔을 때
                        ...
                        BM_zero = nnz(output_one(Prev_state_zero, :) - received_vector(t-1, :));
                        BM_one  = nnz(output_one(Prev_state_one, :)  - received_vector(t-1, :));
                        [Path_metric(1+Current_state, t), I] = min([Path_metric(Prev_state_zero, t-1) + BM_zero, ...
                            Path_metric(Prev_state_one, t-1) + BM_one]);
            ...
            elseif t > num_message_bit + 1
                for j = 1 : 2^(num_message_bit + n_mem + 1 -t)
                    ...
                    % 입력 0을 받아서 현재 값이 되었을 경우
                    if Current_state <  n_states / n_inputSymbols
                        BM_zero = nnz(output_zero(Prev_state_zero, :) - received_vector(t-1, :));
                        BM_one  = nnz(output_zero(Prev_state_one,  :) - received_vector(t-1, :));
...
%% Soft decoding function
    function decoded_output = soft_vitdec(received_output)
        ...
        %---------------------------------------------------------------------------------
        % input을 2개씩 끊어서 received_vector 구성 (R = 1/2 이므로)
        received_vector = reshape(received_output, [n_inputSymbols, num_message_bit + tail_bit]);
        received_vector = received_vector';
        %---------------------------------------------------------------------------------
        % # of states X (num_message_bit + tail bit + 1) Path metric 저장 위한 matrix
        Path_metric = inf(n_states, num_message_bit + tail_bit + 1);
