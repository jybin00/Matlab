...
%--------------------------------------------------------------------------------------
    Path_metric = inf(4, num_message_bit+2 + 1);                               % 64 X num_message_bit + 6 + 1 Path metric 저장 위한 matrix
    ...
    for t = 1 : num_message_bit+2+1                                              % tail bits 존재
        ...
        elseif t > 1 && t <= 2                                                             % t = 2~2 까지는 state가 펼쳐지는 시간
        ...
        elseif t > num_message_bit + 1	       % 마지막 2 state는 접히는 시간
            ...
            for j = 1 : 2^(num_message_bit + 2 + 1 -t)			
                ...
                Prev_Path_zero = mod(2*Current_path, 4)+1;             % prev_path_from 0 = [0 1 1 0 0 0] 끝자리 0
                Prev_Path_one  = mod(2*Current_path + 1, 4) +1;      % prev_path_from 1 = [0 1 1 0 0 1] 끝자리 1
