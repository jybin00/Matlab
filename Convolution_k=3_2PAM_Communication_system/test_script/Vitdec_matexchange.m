...
numSymPerFrame = 25; % Number of QAM symbols per frame

berEstSoft = zeros(size(EbNoVec)); 
berEstHard = zeros(size(EbNoVec));

% trellis = poly2trellis(7,[171 133]);
trellis = trellis(3, [6 7]);
tbl = 36;
rate = 1/2;
...
hold on
semilogy(EbNoVec, qfunc(sqrt(2*10.^(EbNoVec/10) ) ), 'r--' );  %% coding gain은 고려 X modulation만 고려.
%hold on
legend('Soft','Hard','Uncoded','theorical','location','best')
grid
xlabel('Eb/No (dB)')
