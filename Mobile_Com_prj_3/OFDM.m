...
L_d = 0; % Length of delay
L_CP = 20; % Length of CP
...
h_fading = (randn(1, NumSymbol) + 1i*randn(1, NumSymbol))/sqrt(2);
...
noise_power = 10^(-SNR_dB/10); % Convert noise power to linear scale 
noise = sqrt(noise_power/2) * (randn(1, NumSymbol) + 1j*randn(1, NumSymbol)); % Generate complex Gaussian noise
...
channel_gain_dB_multipath = 10*log10(abs(Hk_multipath));
channel_gain_dB_rayleigh = 10*log10(abs(Hk_rayleigh));
...
nexttile
axis([-0.5 0.5 -4 16])
plot(f, channel_gain_dB_multipath)
grid on
box on
title(['Multi path fading frequency response ',num2str(L_d),'*Ts'],'interpreter','latex')
xlabel('Normalized frequency')
ylabel('Magnitude (dB)')

nexttile
axis([-0.5 0.5 -4 16])
plot(f, channel_gain_dB_rayleigh)
grid on
box on
title(['Rayleigh fading frequency response '],'interpreter','latex')
xlabel('Normalized frequency')
ylabel('Magnitude (dB)')
