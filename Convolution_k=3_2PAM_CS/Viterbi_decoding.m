    ...
    for t = 1 : num_message_bit + tail_bit + 1                                       % tail bits 존재
        ...
        elseif t > 1 && t <= tail_bit                                                % t = 2는 state가 펼쳐지는 시간 
            for j = 1 : 2^(t-1)
                ...
                Prev_Path_zero = mod(2*Current_state, 4)+1;            % prev_path_from 0 = [0 0] 끝자리 0
                Prev_Path_one  = mod(2*Current_state + 1, 4) +1;       % prev_path_from 1 = [0 1] 끝자리 1

                % BM = min{ 끝자리 0에서 온 PM + BM, 끝자리 1에서 온 PM + BM}
                if Current_state < 2                                                  % 입력 0을 받아서 현재 값이 되었는가?
                    BM_zero = nnz(output_zero(Prev_Path_zero, :) - codeword(t-1,:));  % 끝자리가 0인 코드에 0이 들어왔을 때 outputd으로 BM 계산
                    BM_one  = nnz(output_zero(Prev_Path_one, :) - codeword(t-1, :));  % 끝자리가 1인 코드에 0이 들어왔을 때
                    Path_metric(1+Current_state, t) = min(Path_metric(Prev_Path_zero, t-1) + BM_zero, ...
                        Path_metric(Prev_Path_one, t-1) + BM_one);
                    Message_bit(1+Current_state, t) = 0;
                else	                                                              % 입력 1을 받아서 현재 값이 되었는가?
                    BM_zero = nnz(output_one(Prev_Path_zero, :) - codeword(t-1, :));
                    BM_one  = nnz(output_one(Prev_Path_one, :) - codeword(t-1, :));
                    Path_metric(1+Current_state, t) = min(Path_metric(Prev_Path_one, t-1) + BM_one, ...
        ...
        elseif t > num_message_bit + 1	       % 마지막 2 state는 접히는 시간
            for j = 1 : 2^(num_message_bit + 2 + 1 -t)			
                ...
                % BM = min{ 끝자리 0에서 온 PM + BM, 끝자리 1에서 온 PM + BM}
                if Current_state < 2                                                     % 입력 0을 받아서 현재 값이 되었는가?
                    BM_zero = nnz(output_zero(Prev_Path_zero, :) - codeword(t-1, :));
                    BM_one  = nnz(output_zero(Prev_Path_one, :) - codeword(t-1, :));
...
% back tracing
    ...
    for t = num_message_bit + tail_bit :-1 : 1
        prev_back_path0 = mod(2*current_state, 4)+1;  
        prev_back_path1 = mod(2*current_state + 1, 4) +1;
        [~, I] = min([Path_metric(prev_back_path0, t), ...
                      Path_metric(prev_back_path1, t)]);
        if I == 1  % 앞에 꺼!
