...
parfor Eb_db = 8: Eb_db_final          % Eb를 바꿔가면서 계산
    for j = 1 : N_frame                       % frame 개수만큼 계산
        ...
        for i = 1: (N_m_bit+2)
            ...
            modulated_output(1, i) = four_QAM( encoded_input(i, :), 2*10^(Eb_db/10) );
        end
        ...
        decoding = Viterbi_soft_decoding(received_signal, N_m_bit, 2*10^(Eb_db/10));
...
legend('Uncoded 4QAM FER', 'Soft Viterbi v = 2, m = 50', 'Soft Viterbi v = 2, m = 50')