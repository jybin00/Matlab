        ...
        elseif t > 1 && t <= 2                                                             % t = 2는 state가 펼쳐지는 시간 
            for j = 1 : 2^(t-1)
                ...
                if Current_path < 2                                                      % 입력 0을 받아서 현재 값이 되었는가?
                    BM_zero = sum((output_zero(Prev_Path_zero, :) - codeword(t-1,:)).^2); % 끝자리가 0인 코드에 0이 들어왔을 때 outputd으로 BM 계산
                    BM_one  = sum((output_zero(Prev_Path_one, :) - codeword(t-1, :)).^2);  % 끝자리가 1인 코드에 0이 들어왔을 때
                    ...
                    BM_zero = sum((output_one(Prev_Path_zero, :) - codeword(t-1,:)).^2);
                    BM_one  = sum((output_one(Prev_Path_one, :) - codeword(t-1, :)).^2);
        ...
        elseif t > num_message_bit + 1	       % 마지막 2 state는 접히는 시간
            for j = 1 : 2^(num_message_bit + 2 + 1 -t)			
                ...
                if Current_path < 32                                                    % 입력 0을 받아서 현재 값이 되었는가?
                    BM_zero = sum((output_zero(Prev_Path_zero, :) - codeword(t-1,:)).^2);
                    BM_one  = sum((output_zero(Prev_Path_one, :) - codeword(t-1, :)).^2);
